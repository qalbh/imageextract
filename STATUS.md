...
- [x] `client-zip` streaming assembly in the browser (MIT, 2.5.0) — src/lib/zip.ts; verified by the verify:results ZIP scenarios: a real archive parsed from disk (EOCD entry count), FS-Access path streams to a picker-chosen file on desktop Chromium, Blob path everywhere else. Device pass pending (the disk-backed-Blob assumption is what it tests).
